...
import type { ApiError, CreateGoalRequest, UpdateGoalRequest } from '../services/types';
...
// Async thunks
const fetchGoals = createAsyncThunk('goals/fetchGoals', async (_, { rejectWithValue }) => {
  try {
...
const deleteGoal = createAsyncThunk('goals/deleteGoal', async (id: number, { rejectWithValue }) => {
  try {
...
export const { clearError, addGoal, removeGoal, toggleGoalCompletion, updateGoal } =
  goalSlice.actions;

export { fetchGoals, createGoal, updateGoalAsync, deleteGoal, toggleGoalCompletionAsync };
